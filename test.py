...
train_num = 200
Y2_train = Y2[:train_num-1]
Y2_test = Y2[train_num:]
Y2_train, max_val, min_val = normalization(Y2_train)
Y2_train = Y2_train.to(dtype=torch.float32)
...
train_inout_seq2 = create_inout_sequences(Y2_train, batch)
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
loss_function = nn.MSELoss()

for epoch in range(epochs+1):
    for features, labels in train_inout_seq2:
        optimizer.zero_grad()
...
future_pred = 61 + len(Y2_test)
data = Y2_train[-batch:].tolist()
...
for i in range(len(Y2_test)):
    test_loss += abs(Y2_test[i] - data[batch+i])
test_loss = test_loss/len(Y2_test)
print(test_loss)

data = move(data, Y2_test, test_loss)
...
with open("file.csv", "w", encoding="utf-8", newline="") as f:
    ...
    plt.plot(np.arange(len(Y2_test)), Y2_test, color='red')
    plt.show()

    csv_writer.writerow(data[batch+len(Y2_test):])
